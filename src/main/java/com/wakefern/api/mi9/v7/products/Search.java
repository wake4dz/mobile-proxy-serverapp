...
public class Search extends BaseService {
	...
	public Search() {
		this.requestPath = MWGApplicationConstants.Requests.Products.prefix
				+ MWGApplicationConstants.Requests.Products.prodSearch;
	}
	...
	public Response getResponse(@PathParam(MWGApplicationConstants.Requests.Params.Path.storeID) String storeID,
			...
			// MWG Docs claim this is optional, but will return an HTTP 400 if it's missing.
			@DefaultValue("100") @QueryParam(MWGApplicationConstants.Requests.Params.Query.take) String takeCount,
		...
		try {
			...
			this.queryParams.put(MWGApplicationConstants.Requests.Params.Query.filters,
					searchFilterStr.isEmpty() ? searchFilterStr
							: searchFilterStr.substring(0, searchFilterStr.length() - "&fq=".length()));
			...
			String jsonResponse = this.mwgRequest(BaseService.ReqType.GET, null, MI9TimeoutRegistry.PRODUCTS_SEARCH,
					MI9TimeoutRegistry.getTimeout(MI9TimeoutRegistry.PRODUCTS_SEARCH));

			if (LogUtil.isUserTrackOn) {
				if ((userID != null) && LogUtil.trackedUserIdsMap.containsKey(userID.trim())) {
					String trackData = LogUtil.getRequestData("storeId", storeID, "isMember", isMember, "excludedProds",
							excludedProds, "searchFilters", searchFilters, "searchTerm", searchTerm, "skipCount",
							skipCount, "sortOrder", sortOrder, "searchBySound", searchBySound, "takeCount", takeCount,
							"userID", userID, "sessionToken", sessionToken, "accept", accept, "contentType",
							contentType);
			...
			String errorData = LogUtil.getRequestData("exceptionLocation", LogUtil.getRelevantStackTrace(e), "storeId",
					storeID, "isMember", isMember, "excludedProds", excludedProds, "searchFilters", searchFilters,
					"searchTerm", searchTerm, "skipCount", skipCount, "sortOrder", sortOrder, "searchBySound",
					searchBySound, "takeCount", takeCount, "userID", userID, "sessionToken", sessionToken, "accept",
