...
public class GetCartItemLocator extends BaseService {
	private final static Logger logger = Logger.getLogger(GetCartItemLocator.class);

	@GET
	@Produces(MWGApplicationConstants.Headers.generic)
    @Path("/cart/{storeId}")
	public Response updateShoppingCartItemLocator(
			@PathParam("storeId") String storeId,
			String jsonData)
	{
		try {
			
			// updateCartItems() is to call Wakefern's ItemLocator API to get additional data 
			return this.createValidResponse(updateCartItems(storeId, jsonData));
			
		} catch (Exception e) {
			LogUtil.addErrorMaps(e, MwgErrorType.PROXY_ITEMLOCATOR_GET_CART_ITEM_LOCATOR);
     ...
     * 
     */
	private String updateCartItems(String storeId, String cartResponseData) {
		try {
