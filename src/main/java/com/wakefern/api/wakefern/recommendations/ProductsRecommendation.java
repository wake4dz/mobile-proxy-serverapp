package com.wakefern.api.wakefern.recommendations;
...
public class ProductsRecommendation extends BaseService {
	...
	public Response getInfoResponse(@PathParam("externalStoreId") String externalStoreId,
			@DefaultValue("") @PathParam("email") String email, @PathParam("ppc") String ppc,
		...
		try {
			...
			this.requestPath = ApplicationConstants.Requests.Recommendations.ProductRecommendationsv2 + "/"
					+ externalStoreId + "/email/" + email + "/fsn/" + ppc;
			...
			/**
			 * DATA FROM V1 "Aisle": "", "Brand": "Bananas", "Category": "Produce", "Name":
			 * "Yellow", "Sku": "000000040112", "ProductId": "179860", "RegularPrice":
			 * "$0.59", "CurrentPrice": "$0.39/ea (avg.)", "Size": "0.8 lb", "InStock":
			 * true, "Imagelink":
			 * "http://content.shoprite.com/legacy/productimagesroot/SJ/0/179860.jpg",
			 * "Note": "On Sale! Reduced Price! $0.10 off Min Qty: 1, Limit: 12 Items",
			 * "DateText": "Sale price valid from Sunday, Mar 11, 2018 until Saturday, Mar
			...
			String skusResp = getProductBySku.getInfo(storeId, skuList, sessionToken, reservedTimeslot);
			...
			for (Item item : pdSKUDao.getItems()) {
				...
				recmd.setAisle(item.getAisle());
				recmd.setBrand(item.getBrand());
				recmd.setCategory(item.getCategory());
				recmd.setName(item.getName());
				recmd.setSku(item.getSku());
				recmd.setProductId(Integer.parseInt(item.getId()));
				recmd.setSize(item.getSize());
				recmd.setInStock(item.getInStock());
				for (ImageLink imgLnk : item.getImageLinks()) {
					if (imgLnk.getRel().equalsIgnoreCase("large")) {
						recmd.setImagelink(imgLnk.getUri());
					}
				}
				recmd.setItemKey(item.getItemKey());
				recmd.setItemType(item.getItemType());
				recmd.setRegularPrice(item.getRegularPrice());
				recmd.setCurrentPrice(item.getCurrentPrice());
			...
			String errorData = LogUtil.getRequestData("exceptionLocation", LogUtil.getRelevantStackTrace(e),
					"externalStoreId", externalStoreId, "email", email, "ppc", ppc, "storeId", storeId, "sessionToken",
					sessionToken, "accept", accept, "contentType", contentType, "reservedTimeslot", reservedTimeslot);
