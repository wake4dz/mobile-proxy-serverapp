package com.wakefern.apim.smsenrollment;
...
public class SmsEnrollment extends BaseService{
	...
	@Path("/")
	@ValidatePPC
	public Response getEnrollmentInfo(@PathParam("ppc") String fsn,
									  @QueryParam("phoneNumber") String phoneNumber)
	{
		...
		try {
			...
			builder.setPath( WakefernApplicationConstants.SmsEnrollment.Upstream.Path)
					.setParameter("frequentShopperNumber", fsn)
	...
	public Response upsertEnrollment(@PathParam("ppc") String fsn, String jsonBody) {
		...
		try {
			...
			try {
				...
				logger.error("SmsEnrollment$updateEnrollment::Exception -> JSONException creating json object from request payload: "
						+ ex.getMessage());
			...
			builder.setPath(WakefernApplicationConstants.SmsEnrollment.Upstream.Path);
			final String requestURI = builder.build().toString();
			String response = HTTPRequest.executePostJSON(requestURI, payload.toString(), headerMap, VcapProcessor.getApiLowTimeout());
